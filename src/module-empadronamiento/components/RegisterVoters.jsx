...
export const RegisterVoters = ({ status = "", isLoading, datos, isLoadingVotantes }) => {
	...
	const searching = (data, buscador) => {
		const newData = data.filter((votante) => {
			if (votante.curp.toUpperCase().includes(buscador.toUpperCase())) return votante;
			if (votante.nombreVotante.toUpperCase().includes(buscador.toUpperCase()))
				return votante;
			if (votante.apellidoMVotante?.toUpperCase().includes(buscador.toUpperCase()))
				return votante;
			if (votante.apellidoPVotante?.toUpperCase().includes(buscador.toUpperCase()))
				return votante;
		});
	...
	const crearBoletas = async () => {
		...
		dispatch(crearBoletasConsulta(info, abrirCerrarModalCBoletas));
	};
	...
	const columns = [
		...
		{ field: "apellidoMVotante", headerName: "SEGUNDO AP.", flex: 3 },
		{
			...
			renderCell: ({ row }) =>
				...
				row.status ? (
					<Typography sx={{ fontSize: "12px", fontWeight: "bold", color: "#048F37" }}>
						COMPLETADO
					</Typography>
				) : (
					<Typography sx={{ fontSize: "12px", fontWeight: "bold", color: "#B50909" }}>
						SIN COMPLETAR
					</Typography>
				),
		},

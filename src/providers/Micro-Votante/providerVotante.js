...
export const postVotanteProvider = async (data) => {
    return VotanteApi.post(`/registrard`, data).then((response) => {
        ...
        return { ok: true, mensaje:response.data.mensaje, data: response.data.data, errorMessage: "" };
    }).catch((error) => {
        console.log(error);
        return { ok: false, data: "", errorMessage: error.message };
    })
...
export const postVotanteJornadaGranelProvider = async (data) => {
    ...
    // return { ok: true, data: [], errorMessage: "" };
    return JornadaVotanteApi.post(`asociar/jornada/votantes`, data).then((response) => {
        // console.log(response)
        return { ok: true, data: response.data.data, errorMessage: "" };
    }).catch((error) => {
        console.log(error);
